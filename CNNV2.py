...
class CNNNet(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, 16, 3, 1),  # 16, 18, 18
            # 输入通道数为1，输出通道数为16，卷积核大小3*3，步长为1
            # 归一化处理，避免梯度消失，加快训练
            nn.BatchNorm2d(16),
            nn.PReLU(),
            nn.MaxPool2d(2, 1)
...
with open('Data/train_set.bin', 'rb') as f:
    ...
    for i in range(len(dBZ)):
        reshapedBZ.append((dBZ[i][2:23, 2:23]).tolist())  # 将25*25重塑为20*20
...
with open('Data/test_set.bin', 'rb') as f:
    ...
    for i in range(len(tdBZ)):
        treshapedBZ.append((tdBZ[i][2:23, 2:23]).tolist())  # 将25*25重塑为20*20
...
if LOAD_MODEL:
    model = torch.load('model/modelV2.pth')
else:
    model = CNNNet()
...
if not LOAD_MODEL:  # 训练
    ...
    torch.save(model, 'model/modelv2.pth')  # 保存训练后的模型
...
with open('Result/CNNResult/dbzv20.pkl', 'wb') as dbzv2:
    pkl.dump(avg_tdBZ, dbzv2)  # 平均dBZ值保存在pkl文件中
with open('Result/CNNResult/obv20.pkl', 'wb') as obv2:
    pkl.dump(real_tRI, obv2)  # 保存真实降水强度
with open('Result/CNNResult/estv20.pkl', 'wb') as estv2:
    pkl.dump(out, estv2)  # 保存估算降水强度
with open('Result/CNNResult/trainv20.pkl', 'wb') as trainv2:
        pkl.dump(loss_train, trainv2)  # 保存训练损失
with open('Result/CNNResult/testv20.pkl', 'wb') as testv2:
        pkl.dump(loss_test, testv2)  # 保存测试损失
