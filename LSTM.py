...
with open('Data/test_set.bin', 'rb') as f:
    ...
    for i in range(len(tdBZ)):
        ...
        lineardBZ = lineardBZ.reshape(-1)
        treshapedBZ.append(lineardBZ.tolist())  # 将25*25重塑为15*15
...
if LOAD_MODEL:
    model = torch.load('model/modelLSTM.pth')
else:
    model = CNNNet()
...
if not LOAD_MODEL:  # 训练
    ...
    torch.save(model, 'model/modelLSTM.pth')  # 保存训练后的模型
...
'''with open('Result/BPResult/dbzLSTM.pkl', 'wb') as dbzLSTM:
    pkl.dump(avg_tdBZ, dbzLSTM)  # 平均dBZ值保存在pkl文件中
with open('Result/BPResult/obbp.pkl', 'wb') as obbp:
    pkl.dump(real_tRI, obbp)  # 保存真实降水强度
with open('Result/BPResult/estbp.pkl', 'wb') as estbp:
    pkl.dump(out, estbp)  # 保存估算降水强度
with open('Result/BPResult/trainbp.pkl', 'wb') as trainbp:
    pkl.dump(loss_train, trainbp)  # 保存真实降水强度
with open('Result/BPResult/testbp.pkl', 'wb') as testbp:
    pkl.dump(loss_test, testbp)  # 保存估算降水强度'''
